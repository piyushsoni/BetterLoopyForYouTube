...
//***************** http://userscripts-mirror.org/scripts/show/62718 ************************************************* //
        ...
        show:function(callback) {
            ...
            if(typeof(Config.styleDrawn) == 'undefined') {                // apply styling
                GM_addStyle("\
                        ...
                        #ConfigBody { width:400px; margin:auto !important; top:50px; right: 0px; position:fixed; text-align:left; background:#f9f9f9; border:1px outset #333; padding:0 !important; font-family:Arial; font-size:14px; border-radius:5px; -moz-border-radius:5px; cursor:default; z-index:99999 !important; color:#333; padding-bottom:1em !important; } \
            ...
            noticeBg.style.height = (window.scrollMaxY + window.innerHeight) + 'px';
                ...
                switch(field.type) {
                    case 'checkbox':
                        if(typeof(field.value) == 'undefined' || !field.value)
                            ...
                            return GM_getValue(key, field.value);
                        break;
                    case 'select':
                    case 'password':
                    case 'text':
                        return GM_getValue(key, (typeof(field.value) != 'undefined' ?  field.value : ''));
                        break;
                    default:
                        return 'not found';
                }   
...
function main()   
{
    ...
    function initLoopy()
    {
        if (!elementsAdded && !buttonLess)
        {
		    ...
		    {
			loopByDefault = true;
			log('loop ON remembered, turning loopByDefault on');
		    }
		    else
			dontLoopRemembered = true;
        ...
        try
        {
			...
			var toPlayNextVideos = (extensionEnabled && nextVideosEnabled && inputbox && inputbox.value != "");
            if(ytObj && !ytLoop && (toPlayNextVideos || loopByDefault || customTime))
            {
                //Make it automatically on since there are next videos to be played or the user wants it that way.
				if(toPlayNextVideos || !dontLoopRemembered)
				{
                LoopyOnOff();
					log('Loop is on by default');
				}
				else 
					log('Not looping by default since not to loop is remembered, or toPlayNextVideos is true');
            ...
            if(ytObj && customTime && eventRegistered)
            {
                ...
                // alert(starttime);
                var callBack = onPlayerStateChange;
                loopCounter--;
    ...
    function SetupActionsUI(addToElement)
    {
        ...
        var sendLinkTextBox = create('input', [['id','idTextBoxDivSendLoopLink'],['type','text'],['style','vertical-align:center;width:90%;padding:2px']], APPEND_MODE_END, divSendLoopLink);
	...
	//Please note, it is VERY important to export this function to the window for it to work. 
	var exportFunc = (typeof GM_exportFunc != "undefined" && GM_exportFunc) ? GM_exportFunc : (typeof exportFunction != "undefined" ? exportFunction : null);
	if(exportFunc)
		exportFunc(onPlayerStateChange, window, {defineAs: "onPlayerStateChange"});
	else
		log("Won't be able to loop, exportFunction not supported");
    ...
    {
        var lastRememberedString = GM_getValue(GM_SAVE_LOOP_STATE,"");
    ...
    function OnActionItemClick(obj)
    {
        ...
        else if(id == 'ActionItemRememberThisLoop')
        {
            ...
            rememberString += (txtStartTime ? txtStartTime.value : "") + ",";
            rememberString += (txtEndTime ? txtEndTime.value : "") + ",";
            ...
            if(rememberString.length > 0)
            {
                ...
                {
                    GM_setValue(GM_SAVE_LOOP_STATE, out.lastRememberedLoopString.replace(out.videoLoopString, rememberString));
                }
                else
                    GM_setValue(GM_SAVE_LOOP_STATE, out.lastRememberedLoopString + rememberString + ";");
            }       
        }
    ...
    //Get other prefs.
    GM_setValue('configDebug', debug);

    var singleButtonFloatLeft = GM_getValue('singleButtonFloatLeft', false);
    GM_setValue('singleButtonFloatLeft', singleButtonFloatLeft); //this will just write the value if not found.
